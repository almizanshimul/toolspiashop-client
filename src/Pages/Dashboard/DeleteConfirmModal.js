...
const DeleteConfirmModal = ({ deletingOrder, refetch }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://toolspiashop.herokuapp.com/tools/${toolId}`)
      .then((res) => setTool(res.data));
  }, [toolId]);
  const handleDelete = () => {
    axiosPrivate
      .delete(
        `https://toolspiashop.herokuapp.com/order/${_id}?toolId=${toolId}&newQuantity=${parseInt(tool?.availableQuantity) + parseInt(quantity)
        }`
      )
      .then((res) => {
